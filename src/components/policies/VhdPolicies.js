...
const VhdPolicies = () => {
  const vhdPolicies = useStaticQuery(graphql`
    {
      allFile(
        filter: { sourceInstanceName: { eq: "pdfspoliciesvhd" } }
        sort: { fields: name }
      ) {
        edges {
          node {
  ...
  return (
    <DocumentsContainer>
      {vhdPolicies.allFile.edges.map((file, index) => {
        return (
