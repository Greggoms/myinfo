...
const SharedPolicies = () => {
  const sharedPolicies = useStaticQuery(graphql`
    {
      allFile(
        filter: { sourceInstanceName: { eq: "pdfspoliciesshared" } }
        sort: { fields: name }
      ) {
        edges {
          node {
  ...
  return (
    <DocumentsContainer>
      {sharedPolicies.allFile.edges.map((file, index) => {
        return (
