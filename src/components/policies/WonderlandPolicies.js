...
const WonderlandPolicies = () => {
  const wonderlandPolicies = useStaticQuery(graphql`
    {
      allFile(
        filter: { sourceInstanceName: { eq: "pdfspolicieswonderland" } }
        sort: { fields: name }
      ) {
        edges {
          node {
  ...
  return (
    <DocumentsContainer>
      {wonderlandPolicies.allFile.edges.map((file, index) => {
        return (
...
export default WonderlandPolicies
