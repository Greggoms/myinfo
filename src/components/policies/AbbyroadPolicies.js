...
const AbbyroadPolicies = () => {
  const abbyroadPolicies = useStaticQuery(graphql`
    {
      allFile(
        filter: { sourceInstanceName: { eq: "pdfspoliciesabbyroad" } }
        sort: { fields: name }
      ) {
        edges {
          node {
  ...
  return (
    <DocumentsContainer>
      {abbyroadPolicies.allFile.edges.map((file, index) => {
        return (
...
export default AbbyroadPolicies
